...
instance Bits#(RegName, 6);
  function Bit#(6) pack(RegName r) = {r.fpoint ? 1 : 0, r.name};
  function RegName unpack(Bit#(6) v) = RegName{name: v[4:0], fpoint: v[5] == 1};
endinstance

// zeroReg is a read-only register set to zero
RegName zeroReg = RegName{name: 0, fpoint: False};
...
function Maybe#(Optype) opcodeType(Opcode opcode);
  return case (opcode)
    ...
    7'b0000111 : Valid(Itype); // LOAD-FP
    ...
    7'b0100111 : Valid(Stype); // STORE-FP
    ...
    7'b1000011 : Valid(R4type);// MADD
    7'b1000111 : Valid(R4type);// MSUB
    7'b1001011 : Valid(R4type);// NMSUB
    7'b1001111 : Valid(R4type);// NMADD
    7'b1010011 : Valid(Rtype); // OP-FP
  ...
  case (opcode(instr))
    ...
    7'b0001111 :
      if (immediateBits(instr)[11:8] == 0 &&
          getRs1(instr.bits) == 0 && getRd(instr.bits) == 0)
        return
...
instance HasRegister1#(Instr);
  function RegName register1(Instr instr);
    return case (hasRegister1(instr)) matches
      tagged Valid .r : RegName{name: r, fpoint: hasFloatRs1(instr)};
      default : zeroReg;
    endcase;
  endfunction
endinstance
...
instance HasRegister3#(Instr);
  function RegName register3(Instr instr);
    return case (hasRegister3(instr)) matches
      tagged Valid .r : RegName{name: r, fpoint: hasFloatRs3(instr)};
      default : zeroReg;
    endcase;
  endfunction
endinstance
